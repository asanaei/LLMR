...
safe({
  ...
  res <- call_llm_structured(cfg, c(
    system = "Reply as JSON only.",
    user   = "Produce fields name and score about 'MNIST'."
  ), schema = list(type="object", properties=list(name=list(type="string"), score=list(type="number")), required=list("name", "score"), additionalProperties=FALSE))
