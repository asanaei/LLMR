...
Let us see if there is first name bias in a sepcific model. For this example we 
use the `gpt-4.1-nano` model which is fast and relatively cheap.
...
config <- llm_config(
  ...
  max_tokens = 10  # Very few tokens as requested
)
```
...
results$response_text_clean |> head()
