...
This vignette demonstrates how to use LLMR for embedding analysis of
presidential inaugural speeches.

**Note:** This vignette requires a valid Gemini API key and will not run
during package installation, and you may need to change the model
name.\*\*
...
We’ll analyze excerpts from several U.S. presidential inaugural
addresses:
...
When run with a valid API key, this analysis would:

1.  Generate high-dimensional embeddings for each presidential speech
    excerpt
2.  Show correlation patterns between different presidents’ rhetorical
    styles
3.  Create a dendrogram clustering presidents by linguistic similarity
4.  Reveal historical and stylistic patterns in presidential rhetoric

**To run this vignette yourself:** Set your Gemini API key with
`Sys.setenv(GEMINI_API_KEY = "your_key_here")` before running the code
chunks.
