...
cfg_openai_json <- enforce_json_mode(cfg_openai, schema = schema, strict = TRUE)

# Anthropic: FORCE tool-only JSON
cfg_anthropic_json <- enforce_json_mode(cfg_anthropic, schema = schema)
...
cfg_gemini_json <- enforce_json_mode(cfg_gemini, schema = NULL)
...
print(out_json |> select(sentence, family, confidence, json_ok))
...
res_par <- experiments |>
  ...
  call_llm_par_json(schema = schema ,
                    # these are the fields kept
...
print(vec_out %>% select(response_text, family, confidence, json_ok))


vec_out2 <- llm_fn_json(
  x       = df$sentence,
  prompt  = "For '{x}', Return the taxonomic family of the animal in the sentence,
  or NA if none; return JSON with keys: family, confidence (\\in [0,1]), rationale.",
...
print(vec_out2 %>% select(response_text, family, confidence, json_ok))


vec_out3 <- llm_fn_json(
  x       = df$sentence,
  prompt  = "For '{x}', Return the taxonomic family of the animal in the sentence,
...
print(vec_out3 |> select(rationale, family, confidence, json_ok))
